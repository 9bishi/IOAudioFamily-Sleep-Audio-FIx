...
bool sleepMute = false;
UInt16 status, cmdCounter = 0;
void startUpdate();
//
...
IOReturn IOAudioDevice::protectedCompletePowerStateChange()
{
    ...
    if (currentPowerState != pendingPowerState) {
        ...
        if (currentPowerState == kIOAudioDeviceSleep) {	
            ...
            //Setting Sleep Mute to true after sleep so we can send the command
            sleepMute = true;
        }
...
void updateEAPD() {
    ...
    IOLog("EAPD: Sending Command\n");
    UInt32 cmd;
        if(cmdCounter == 0)
        cmd = 0x01470c02;//Speaker EAPD
    else
        cmd = 0x02170c02;//Headphone EAPD
    ...
    for (int i = 0; i < 1000; i++) {
        ::IODelay(100);
        ioreg_->readBytes(0x68, &status, sizeof(status));//Checking the Status of Previous Write
        if (status & 0x2)
            break;
    }
    if(status & 0x2)
    {
        ...
        updateEAPD();
        sleepMute = false;
        IOLog("EAPD:  Command Sent Successfully\n");
        cmdCounter =0;
    }
    
}

//Starting with the Path for EAPD Verb Sending
void startUpdate() {
...
void IOAudioDevice::EAPDVerbUpdate()
{
    startUpdate();
}